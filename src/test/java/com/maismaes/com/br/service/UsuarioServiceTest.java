...
class UsuarioServiceTest {
  ...
  @Mock private com.maismaes.com.br.utils.UserValidationUtils userValidationUtils;

  @Mock private com.maismaes.com.br.repository.PerfilRepository perfilRepository;
  ...
  class CadastrarUsuario {
    ...
    void devePropagarExcecaoQuandoRepositorioFalhar() {
      ...
      RuntimeException ex =
          assertThrows(RuntimeException.class, () -> usuarioService.cadastrarUsuario(entrada));
  ...
  class BuscarDadosConta {
    ...
    void deveRetornarDadosDoUsuario() {
      ...
      Perfil perfil = Perfil.builder().perfilEmail("mae@example.com").role(Role.MAE_SOLO).build();

      Usuario usuario =
          Usuario.builder().nome("Maria").telefone("81999999999").perfil(perfil).build();

      when(usuarioRepository.findByPerfil_PerfilEmail("mae@example.com"))
          .thenReturn(Optional.of(usuario));

      BuscaDadosContaResponseDTO response = usuarioService.buscarDadosConta("mae@example.com");
      ...
      verify(usuarioRepository, times(1)).findByPerfil_PerfilEmail("mae@example.com");
    }
    ...
    void deveLancarExcecaoQuandoUsuarioNaoEncontrado() {
      ...
      verify(usuarioRepository, times(1)).findByPerfil_PerfilEmail("naoexiste@example.com");
    }
  }

  // helpers para criar massa de teste
  private Perfil criarPerfil(String email, String senha) {
    return Perfil.builder().perfilEmail(email).senha(senha).build();
  }
  ...
  private Usuario criarUsuario(UUID id, String nome, String email, String telefone, Perfil perfil) {
    Usuario.UsuarioBuilder builder =
        Usuario.builder().nome(nome).email(email).telefone(telefone).perfil(perfil);
