...
public class InfocardServiceTest {
  ...
  @Mock private InfocardRepository infocardRepository;

  @Mock private UsuarioRepository usuarioRepository;

  @InjectMocks private InfocardService infocardService;
  ...
  void deveBuscarInfocardPorId() {
    ...
    when(infocardRepository.findById(id)).thenReturn(Optional.of(infocard));
  ...
  void deveLancarExcecaoQuandoInfocardNaoExiste() {
    ...
    when(infocardRepository.findById(id)).thenReturn(Optional.empty());

    assertThrows(RuntimeException.class, () -> infocardService.buscarPorId(id));
  }
  ...
  void deveListarTodosQuandoAtivoForNulo() {
    ...
    when(infocardRepository.findAll(any(Pageable.class))).thenReturn(page);

    Page<InfoCardResponseDto> result = infocardService.listar(0, 10, null);
  ...
  void deveListarPorAtivo() {
    ...
    when(infocardRepository.findByAtivo(eq(true), any(Pageable.class))).thenReturn(page);

    Page<InfoCardResponseDto> result = infocardService.listar(0, 10, true);
  ...
  void deveBuscarPorTitulo() {
    ...
    Page<InfoCardResponseDto> result = infocardService.buscarPorTitulo("teste", 0, 10);
  ...
  void deveAtualizarInfocard() {
    ...
    InfoCardResponseDto result = infocardService.atualizar(id, dto);
  ...
  void deveListarDestaques() {
    ...
    when(infocardRepository.findByDestaqueTrueAndAtivoTrue(any(Pageable.class))).thenReturn(page);

    Page<InfoCardResponseDto> result = infocardService.listarDestaques(0, 10);
  ...
  void deveAdicionarFavorito() {
    ...
    when(infocardRepository.findById(cardId)).thenReturn(Optional.of(infocard));
  ...
  void deveListarFavoritos() {
    ...
    Page<InfoCardResponseDto> result = infocardService.listarFavoritos(userId, 0, 10);
