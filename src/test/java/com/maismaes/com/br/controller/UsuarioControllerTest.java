...
class UsuarioControllerTest {
  ...
  class CadastrarUsuario {
    ...
    void deveCriarUsuarioGerarTokenERetornar201ComToken() {
      var dto =
          new CadastroUsuarioRequestDTO("Maria", "maria@example.com", "81 912345678", "senha");
    ...
    void devePropagarExcecaoQuandoServicoFalhar() {
      var dto = new CadastroUsuarioRequestDTO("Joao", "joao@example.com", "81 912345679", "senha");

      when(bCryptPasswordEncoder.encode("senha")).thenReturn("encoded");
      when(usuarioService.cadastrarUsuario(any(Usuario.class)))
          .thenThrow(new RuntimeException("erro salvar"));

      RuntimeException ex =
          assertThrows(RuntimeException.class, () -> usuarioController.cadastrarUsuario(dto));
  ...
  class BuscarMinhaConta {
    ...
    void deveRetornarDadosDoUsuarioLogado() {
      ...
      when(usuarioService.buscarDadosConta("mae@example.com")).thenReturn(dto);
      ...
      verify(usuarioService, times(1)).buscarDadosConta("mae@example.com");
    }
    ...
    void deveLancarExcecaoQuandoUsuarioNaoEncontrado() {
      ...
      Perfil perfil = Perfil.builder().perfilEmail("naoexiste@example.com").build();

      when(usuarioService.buscarDadosConta("naoexiste@example.com"))
          .thenThrow(new UsuarioNaoEncontradoException());

      assertThrows(
          UsuarioNaoEncontradoException.class, () -> usuarioController.buscarMinhaConta(perfil));

      verify(usuarioService, times(1)).buscarDadosConta("naoexiste@example.com");
    }
  }
}
