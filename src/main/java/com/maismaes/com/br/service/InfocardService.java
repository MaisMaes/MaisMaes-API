...
public class InfocardService {
  ...
  public InfoCardResponseDto cadastrar(CadastroInfocardDto dto, Usuario criador) {
  ...
  public InfoCardResponseDto buscarPorId(UUID id) {
    ...
    Infocard infocard =
        infocardRepository
            .findById(id)
            .orElseThrow(() -> new RuntimeException("Infocard não encontrado"));

    return new InfoCardResponseDto(infocard);
  }

  public Page<InfoCardResponseDto> listar(int pagina, int tamanho, Boolean ativo) {
  ...
  public Page<InfoCardResponseDto> buscarPorTitulo(String titulo, int pagina, int tamanho) {

    Pageable pageable = PageRequest.of(pagina, tamanho, Sort.by("dataCriacao").descending());

    return infocardRepository
        .findByTituloContainingIgnoreCaseAndAtivoTrue(titulo, pageable)
        .map(InfoCardResponseDto::new);
  }

  public InfoCardResponseDto atualizar(UUID id, AtualizaDto dto) {
  ...
  public Page<InfoCardResponseDto> listarDestaques(int pagina, int tamanho) {
  ...
  @Transactional
  public void adicionarFavorito(UUID usuarioId, UUID infocardId) {
  ...
  @Transactional
  public void removerFavorito(UUID usuarioId, UUID infocardId) {
  ...
  @Transactional()
  public Page<InfoCardResponseDto> listarFavoritos(UUID usuarioId, int pagina, int tamanho) {
    ...
    List<InfoCardResponseDto> favoritos =
        usuario.getFavoritos().stream().map(InfoCardResponseDto::new).toList();
    ...
    return new PageImpl<>(conteudo, PageRequest.of(pagina, tamanho), favoritos.size());
  }
}
